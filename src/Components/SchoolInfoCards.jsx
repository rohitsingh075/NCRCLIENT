...
const SchoolInfoCards = () => {
  ...
  return (
    <div className="bg-gray-50">
      {/* Slim Banner Section */}
      <div className="relative bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 py-10 px-6 text-center text-white overflow-hidden rounded-b-2xl shadow-lg">
        {/* Decorative Floating Background */}
        <motion.div
          animate={{ y: [0, -20, 0] }}
          transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
          className="absolute w-72 h-72 bg-red-800 opacity-20 rounded-full blur-3xl -top-10 -left-10"
        ></motion.div>
        <motion.div
          animate={{ y: [0, 20, 0] }}
          transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }}
          className="absolute w-64 h-64 bg-indigo-700 opacity-20 rounded-full blur-3xl bottom-0 right-0"
        ></motion.div>
        ...
        <div className="relative z-10">
          ...
          >
            ...
            >
              ...
              <p className="text-xl font-bold">{schoolDetails.schoolCode}</p>
            </motion.div>
            <motion.div
              variants={item}
              className="bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-md"
            >
              ...
              <p className="text-xl font-bold">{schoolDetails.affiliationNo}</p>
            </motion.div>
            <motion.div
              variants={item}
              className="bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-md"
            >
              ...
              <p className="text-xl font-bold">{schoolDetails.udiseNo}</p>
            </motion.div>
          </motion.div>
        </div>
      </div>
        ...
        {infoCards.map((card, index) => (
          ...
          >
            <div className="p-8 flex flex-col items-center text-center">
              <div
                className={`inline-flex items-center justify-center w-16 h-16 ${card.iconBg} rounded-xl mb-4 shadow-md`}
              >
                {card.icon}
              </div>
