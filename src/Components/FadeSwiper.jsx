...
const FadeSwiper = () => {
  ...
  useEffect(() => {
    const calculateHeight = () => {
      // Assuming navbar is 70px high (adjust this value based on your actual navbar height)
  ...
  return (
    <div className="w-full h-full left-0 right-0">
      <div className="w-full h-full "> {/* mt-16 accounts for navbar height */}
        <Swiper
        ...
        >
          {slides.map((slide) => (
            <SwiperSlide key={slide.id} className="overflow-hidden">
              ...
              <img
                src={slide.image}
                className="w-full h-120 object-fit select-none brightness-70"
                alt={`Slide ${slide.id}`}
              />
              </div>
              {/* Overlay Content */}
              <div className="absolute top-1/8 left-20 text-white space-y-4">
